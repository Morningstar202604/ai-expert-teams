...
class APIScoringEngine:
    ...
    def _extract_api_info(self) -> None:
        ...
        for path_obj in paths.values():
            if isinstance(path_obj, dict):
                endpoint_count += len([m for m in path_obj 
    ...
    def _check_url_structure_consistency(self) -> float:
        ...
        with_slash = sum(1 for path in paths if path.endswith('/'))
    ...
    def _check_sensitive_data_handling(self) -> float:
        ...
        for schema in schemas.values():
            ...
            for prop_name, prop_def in properties.items():
                ...
                if any(sensitive in prop_name.lower() for sensitive in sensitive_field_names):  # noqa: SIM102
    ...
    def _has_link_properties(self, schema: dict[str, Any]) -> bool:
        ...
        return any(prop_name.lower() in link_indicators for prop_name in properties)
        ...
        for path_obj in paths.values():
            ...
            for method, operation in path_obj.items():
                ...
                if has_error_responses:
                    ...
                    for status_code, response in responses.items():
                        if (status_code.startswith(('4', '5'))) and isinstance(response, dict):
                            content = response.get('content', {})
                            for media_obj in content.values():
                                schema = media_obj.get('schema', {})
        ...
        for path_obj in paths.values():
            ...
            if get_operation and isinstance(get_operation, dict):
                ...
                for response in responses.values():
                    ...
                    if any(header.lower() in cache_headers for header in headers):
