...
class APILinter:
    ...
    def __init__(self):
        ...
        self.raw_endpoints: dict | None = None
    ...
    def _validate_openapi_structure(self) -> None:
        ...
        for fld in required_fields:
            if fld not in self.openapi_spec:
                self.report.add_issue(LintIssue(
                    severity='error',
                    category='structure',
                    message=f"Missing required fld: {fld}",
                    path=f"/{fld}",
                    suggestion=f"Add the '{fld}' fld to the root of your OpenAPI specification"
                ))
    ...
    def _validate_info_section(self) -> None:
        ...
        for fld in required_info_fields:
            if fld not in info:
                self.report.add_issue(LintIssue(
                    severity='error',
                    category='documentation',
                    message=f"Missing required info fld: {fld}",
                    path=f"/info/{fld}",
                    suggestion=f"Add a '{fld}' fld to the info section"
                ))
        
        for fld in recommended_info_fields:
            if fld not in info:
                self.report.add_issue(LintIssue(
                    severity='warning',
                    category='documentation',
                    message=f"Missing recommended info fld: {fld}",
                    path=f"/info/{fld}",
                    suggestion=f"Consider adding a '{fld}' fld to improve API documentation"
                ))
    ...
    def _validate_paths_section(self) -> None:
        ...
        self.report.total_endpoints = sum(
            len([method for method in path_obj if method.upper() in self.http_methods])
    ...
    def _validate_path_structure(self, path: str) -> bool:
        ...
        for i, segment in enumerate(segments):
            ...
            if not self.kebab_case_pattern.match(segment):  # noqa: SIM102
    ...
    def _validate_http_method_usage(self, path: str, method: str, operation: dict[str, Any]) -> bool:
        ...
        if method in ['POST', 'PUT', 'PATCH'] and 'requestBody' not in operation:  # noqa: SIM102
    ...
    def _validate_responses(self, path: str, method: str, responses: dict[str, Any]) -> bool:
        ...
        has_error_responses = any(code.startswith(('4', '5')) for code in responses)
        ...
        for schema_name, schema in schemas.items():
            ...
            if not self.pascal_case_pattern.match(schema_name):
                self.report.add_issue(LintIssue(
                    ...
                    suggestion="Use PascalCase for schema names (e.g., 'UserProfile', 'OrderItem')"
                ))
