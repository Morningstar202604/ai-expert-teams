...
try:
    ...
    from search_client import SearchEngineError, search
except ImportError:
    search = None
...
class QueryAnalyzer:
    ...
    def analyze(self, topic: str, focus_areas: list[str] | None = None) -> dict[str, Any]:
        """分析查询，返回结构化信息"""
...
class SearchOrchestrator:
    ...
    def _should_continue(self, results: dict, current_round: int, max_rounds: int) -> bool:
        ...
        return bool(total_results < 5 and current_round < max_rounds)
...
class ReportGenerator:
    ...
    def generate_markdown(self, research: dict) -> str:
        ...
        lines = [
            f"# 深度研究报告：{topic}",
            "",
            f"**生成时间：** {datetime.now().strftime('%Y-%m-%d %H:%M')}",  # noqa: DTZ005
        ...
        lines.append("")
        lines.append("---")
        lines.append("*由 deep-research v1.0 生成*")
