...
def validate_compose(parsed, strict=False):
    ...
    for name, svc in services.items():
        ...
        if "healthcheck" not in svc:
            findings.append({
                ...
                "message": "No healthcheck defined — orchestrator can't detect unhealthy state",
                "service": name,
            })
        ...
        if isinstance(env, list):
            for e in env:
                e_str = str(e)
                if re.search(r"(?:PASSWORD|SECRET|TOKEN|KEY)=\S+", e_str, re.IGNORECASE) and "env_file" not in svc:
                    findings.append({
                        "severity": "critical",
                        "category": "security",
        ...
        if isinstance(volumes, list):
            for v in volumes:
                v_str = str(v)
                if v_str.startswith(("./", "/")) and "/var/run/docker.sock" in v_str:
                    findings.append({
                        "severity": "critical",
                        "category": "security",
    ...
    # Network check
    if ("networks" not in parsed or not parsed["networks"]) and len(services) > 1:
        findings.append({
            "severity": "low",
            "category": "networking",
